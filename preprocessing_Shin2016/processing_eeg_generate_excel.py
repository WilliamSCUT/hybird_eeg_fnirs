...
if __name__ == "__main__":
    ...
    for n in range(1, count+1):  ############## per subject ##############
        ...
        for window_start in windows:  ############## per window ############
            ...
            trial_att = np.array(compute_(current_window_data))  # (60,)
        ...
        print(f'rest std {np.std(rest_arr, axis=0)} for subject {n}')
        print(f'25% percentile of rest {np.percentile(rest_arr, 25)} for subject {n}')
        print(f'75% percentile of rest {np.percentile(rest_arr, 75)} for subject {n}')
        print(f'mean of rest {np.mean(rest_arr)} for subject {n}')
        print(f'median of rest {np.median(rest_arr)} for subject {n}')

        print(f'arithmetic std {np.std(arithmetic_arr, axis=0)} for subject {n}')
        print(f'25% percentile of arithmetic {np.percentile(arithmetic_arr, 25)} for subject {n}')
        print(f'75% percentile of arithmetic {np.percentile(arithmetic_arr, 75)} for subject {n}')
        print(f'mean of arithmetic {np.mean(arithmetic_arr)} for subject {n}')
        print(f'median of arithmetic {np.median(arithmetic_arr)} for subject {n}')

        data_records.append({
            "Subject": n,
            "Rest Std": np.std(rest_arr, axis=0)[0],
            "Rest 25% Percentile": np.percentile(rest_arr, 25),
            "Rest 75% Percentile": np.percentile(rest_arr, 75),
            "Rest Mean": np.mean(rest_arr),
            "Rest Median": np.median(rest_arr),
            "Arithmetic Std": np.std(arithmetic_arr, axis=0)[0],
            "Arithmetic 25% Percentile": np.percentile(arithmetic_arr, 25),
            "Arithmetic 75% Percentile": np.percentile(arithmetic_arr, 75),
            "Arithmetic Mean": np.mean(arithmetic_arr),
            "Arithmetic Median": np.median(arithmetic_arr),
        })
    import pandas as pd
    df = pd.DataFrame(data_records)
    output_file = "subject_statistics.xlsx"
