...
class OurEEGProcessor:
    ...
    def process_data(self, ma_file_path, rest_file_path):
        """Process all data"""
        # Load MA (Mental Arithmetic) data
        ma_data = np.load(ma_file_path)
        ma_trials = ma_data['data']
        ma_labels = ma_data['labels']  # 原始代码
        
        # Load REST data
        rest_data = np.load(rest_file_path)
        ...
        for trial_idx in tqdm(range(len(all_trials)), desc="Processing trials"):
        ...
        # Save processed data
        save_path = 'preprocessed_data/features_w3.npz'
...
if __name__ == "__main__":
    ...
    # 数据文件路径
    ma_file = r'preprocessed_data\subject_03\preprocessed_data\trial-160_channel-4_epoch-0-10.0_fs-256.npz'
    rest_file = r'preprocessed_data\subject_03\preprocessed_data\trial-160_channel-4_epoch-0.0-10.0_fs-256_REST.npz'
    processor = OurEEGProcessor(config)
    processor.process_data(ma_file, rest_file) 